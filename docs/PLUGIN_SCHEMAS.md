...
The lifecycle mode is always `on-demand` accompanied by `idleTimeoutSeconds`. **Tools are not started at boot**. Instead, they are spawned dynamically via a `plugin.load` trigger event on their first invocation. A tool must subscribe directly to its own capability `triggerEvent`.
...
| `tags` | string[] | Meta tags for semantic search routing in `CapabilityRegistry` |
...
|---|---|---|
| tool | `capability.tool.<name>` | `CapabilityRegistry` intercepts generic invokes, re-publishes to exact triggerEvent type; tool receives direct UDS frames |
| agent | *(None)* | `CapabilityRegistry` evaluates candidate bids, resolves, and forwards to target agent via `message.{agentId}` |
