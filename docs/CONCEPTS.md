...
[CapabilityInterceptor]    ← resolve provider, run auction, spawn if needed
      ↓
[PluginInterceptor]        ← handle lifecycle events
      ↓
...
Plugins do not need to run all the time. The `PluginInterceptor` manages two lifecycle modes:
...
system.plugin.spawn published
    ↓
PluginInterceptor spawns child process via ProcessBuilder
    ↓
...
[idle for idleTimeoutSeconds]
    ↓
PluginInterceptor kills the process
```
...
**Plugin discovery**: `PluginInterceptor` walks the project directory tree at boot looking for `plugin.json` files. Every file found is parsed and its capabilities are indexed. Hot reloading is triggered by `plugin.installed` or `system.catalog.refresh` events.
...
When `PluginInterceptor` spawns an on-demand plugin, it uses `ProcessBuilder` and redirects stdout/stderr directly to a log file:
...
No component knows about the existence of any other component. `CapabilityInterceptor` does not import `PluginInterceptor`. `SummaryAgent` does not import `WordCountTool`. `ConsolePlugin` does not import `Agent`.
